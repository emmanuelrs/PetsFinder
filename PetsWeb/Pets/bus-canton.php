...
$division = '';
$cantones = $_POST['Canton'];


 if($tipo == "Encontrada"){
        
    $query_procedimiento = oci_parse($conn, "BEGIN :cursor := busquedas.busquedacanton_enc(:busq); END;");  
	$cursor = oci_new_cursor($conn);
	oci_bind_by_name($query_procedimiento,':busq', $cantones);
...
echo $division;
    ...
    $query_procedimiento = oci_parse($conn, "BEGIN :cursor := busquedas.busquedacanton_per(:busq); END;");
    $cursor = oci_new_cursor($conn);
	oci_bind_by_name($query_procedimiento,':busq', $cantones);
	oci_bind_by_name($query_procedimiento,':cursor', $cursor , -1, OCI_B_CURSOR);
	oci_execute($query_procedimiento);
